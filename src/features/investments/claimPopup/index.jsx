...
import { claimBaseTokensForOwnerApiFunction, claimTokensForInvestorApiFunction, investInSTOApiFunction } from 'services/launchpad/launchpadApiFunctions';
...
function ClaimPopup(props) {
    ...
    const onClaimSubmit = async () => {
        ...
        try {
            ...
            const claimTokensForInvestorApiRes = await claimTokensForInvestorApiFunction(claimApiData);
            console.log("claimTokensForInvestorApiRes", claimTokensForInvestorApiRes)
            const claimTokensForInvestorContractRes = await sendTransactionAsync(claimTokensForInvestorApiRes?.data)

            const claimTokensForInvestorContractReceipt = await publicClient.waitForTransactionReceipt({ hash: claimTokensForInvestorContractRes });
            if (claimTokensForInvestorContractReceipt?.status !== "success") {
                throw new Error("Claim Tokens For Investor Contract transaction failed!");
            }

            toast.success(claimTokensForInvestorApiRes?.message);

            if (currentWalletAddress === getSTODetailData?.data?.owner) {
                const claimBaseTokensForOwnerApiRes = await claimBaseTokensForOwnerApiFunction(claimApiData);
                console.log("claimBaseTokensForOwnerApiRes", claimBaseTokensForOwnerApiRes)

                const claimBaseTokensForOwnerContractRes = await sendTransactionAsync(claimBaseTokensForOwnerApiRes?.data)

                const claimBaseTokensForOwnerContractReceipt = await publicClient.waitForTransactionReceipt({ hash: claimBaseTokensForOwnerContractRes });
                if (claimBaseTokensForOwnerContractReceipt?.status !== "success") {
                    throw new Error("Claim Tokens For owner Contract transaction failed!");
                }
                toast.success(claimBaseTokensForOwnerApiRes?.message);
            }
            ...
            >
                ...
                <div className="text_cyan_A100_border mt-[1.25rem] flex items-center gap-[1.00rem] border border-solid bg-gradient2 p-[1rem] sm:flex-col">
                    <Text as="p" className="text-[1.00rem] font-medium leading-[1rem] text-white-a700">
                        Soft Cap investment goal reached successfully
                    </Text>
                </div>
                            ...
                            <div className="mt-[1.38rem]">
                                ...
                                <div className="mt-[0.88rem] flex flex-wrap justify-between gap-[1.25rem]">
                                    ...
                                    >
                                        {baseTokenData?.isSuccess &&
                                            <>
                                                {formatUnits(parseInt(getSTOInvestmentData?.data?.investmentAmount), baseTokenData?.data[2]?.result)}
                                ...
                                <div className="mt-[0.88rem] flex flex-wrap justify-between gap-[1.25rem]">
                                    ...
                                    <Heading
                                        ...
                                        className="text-[0.88rem] font-bold capitalize tracking-[0.00rem] text-white-a700"
                                    >
                                        {(baseTokenData?.isSuccess && stoTokenData?.isSuccess && !getSTODetailLoading) &&
                                            <> {(formatUnits(getSTOInvestmentData?.data?.investmentAmount, baseTokenData?.data[2]?.result) * getSTODetailData?.data?.tokenPriceStoToken) / getSTODetailData?.data?.tokenPriceBaseToken}{" "}{stoTokenData?.data[1]?.result}</>
                                        }
                                    </Heading>
                                </div>
                            </div>
                            ...
                            <div className="mt-[1.38rem]">
                                ...
                                <div className="mt-[0.88rem] flex flex-wrap justify-between gap-[1.25rem]">
                                    <Heading
                                        ...
                                        variants='lightLabel'
                                    >
                                        <strong>{stoTokenData?.isSuccess && stoTokenData?.data[1]?.result}</strong> Deposited
                                    </Heading>
                                    <Heading
                                        size="headingmd"
                                        as="p"
                                    ...
                                    >
                                        {
                                            (baseTokenData?.isSuccess && stoTokenData?.isSuccess) &&
                                            <>
                                                {(formatUnits(getSTODetailData?.data?.hardCap, baseTokenData?.data[2]?.result) * getSTODetailData?.data?.tokenPriceStoToken) / getSTODetailData?.data?.tokenPriceBaseToken}
                                                {" "}
                                                {stoTokenData?.data[1]?.result}
                                            </>
                                        }
                                    </Heading>
                                </div>
                                <div className="mt-[0.88rem] flex flex-wrap justify-between gap-[1.25rem]">
                                    <Heading
                                        ...
                                        variants='lightLabel'
                                    >
                                        <strong>{stoTokenData?.isSuccess && stoTokenData?.data[1]?.result}</strong> Distributed amount
                                    </Heading>
                                    <Heading
                                        size="headingmd"
                                        as="p"
                                    ...
                                    >
                                        {
                                            (baseTokenData?.isSuccess && stoTokenData?.isSuccess) &&
                                            <>
                                                {(formatUnits(getSTODetailData?.data?.raisedAmount, baseTokenData?.data[2]?.result) * getSTODetailData?.data?.tokenPriceStoToken) / getSTODetailData?.data?.tokenPriceBaseToken}
                                                {" "}
                                                {stoTokenData?.data[1]?.result}
                                            </>
                                        }
                                ...
                                <div className="mt-[0.88rem] flex flex-wrap justify-between gap-[1.25rem]">
                                    <Heading
                                        ...
                                        variants='lightLabel'
                                    >
                                        <strong>{baseTokenData?.isSuccess && baseTokenData?.data[1]?.result}</strong> token Claimable amount
                                    </Heading>
                                    <Heading
                                        size="headingmd"
                                        as="p"
                                    ...
                                    >
                                        {
                                            (baseTokenData?.isSuccess) &&
                                            <>
                                                {formatUnits(getSTODetailData?.data?.raisedAmount, baseTokenData?.data[2]?.result)}
                                                {" "}
                                                {baseTokenData?.data[1]?.result}
                                            </>
                                        }
                                ...
                                <div className="mt-[0.88rem] flex flex-wrap justify-between gap-[1.25rem]">
                                    <Heading
                                        ...
                                        variants='lightLabel'
                                    >
                                        <strong>{stoTokenData?.isSuccess && stoTokenData?.data[1]?.result}</strong> Token Claimable amount
                                    </Heading>
                                    <Heading
                                        size="headingmd"
                                        as="p"
                                    ...
                                    >
                                        {
                                            (baseTokenData?.isSuccess && stoTokenData?.isSuccess) &&
                                            <>
                                                {((formatUnits(getSTODetailData?.data?.hardCap, baseTokenData?.data[2]?.result) - formatUnits(getSTODetailData?.data?.raisedAmount, baseTokenData?.data[2]?.result)) * getSTODetailData?.data?.tokenPriceStoToken) / getSTODetailData?.data?.tokenPriceBaseToken}
                                                {" "}
                                                {stoTokenData?.data[1]?.result}
                                            </>
                                        }
